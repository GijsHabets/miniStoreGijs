...
export class ItemService{
  ...
  addItem(item: Item){
    ...
    this.http.post<Item>("http://167.99.16.16:8080/api/minicon/mini", item, {headers}).subscribe(responseData =>{
  ...
  updateItem(newItem: Item, index: number){
    const headers = { 'Content-Type': 'application/json','Authorization': 'Bearer '+sessionStorage.getItem('JWT') };
    const route = "http://167.99.16.16:8080/api/minicon/minis/"+index.toString()
  ...
  getItems(){
    this.http.get<Item[]>("http://167.99.16.16:8080/api/minicon/minis").subscribe(
      (items: Item[]) => {
        this.items = items;
      },
      (error) => {
  ...
  deleteItem(deleteItem: Item) {
    const headers = { 'Content-Type': 'application/json','Authorization': 'Bearer '+sessionStorage.getItem('JWT') };
    const route = "http://167.99.16.16:8080/api/minicon/minis/"+deleteItem.id.toString()
